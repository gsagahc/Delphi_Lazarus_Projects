...
type
  TFrmConsultarPedData = class(TForm)
    Panel1: TPanel;
    Panel2: TPanel;
    DBGrid1: TDBGrid;
...
var
  FrmConsultarPedData: TFrmConsultarPedData;
...
procedure TFrmConsultarPedData.PNGButton2Click(Sender: TObject);
begin
  Close;
end;

procedure TFrmConsultarPedData.PNGButton1Click(Sender: TObject);
begin
  IBQPedidos.Close;
...
procedure TFrmConsultarPedData.DBGrid1DblClick(Sender: TObject);
begin
...
procedure TFrmConsultarPedData.PNGButton6Click(Sender: TObject);
begin
...
procedure TFrmConsultarPedData.DBGrid1CellClick(Column: TColumn);
begin
  If  Pedido=Nil Then
    Pedido:=TPedido.Create;
  Pedido.Id_pedido:=IBQPedidosID_PEDIDO.AsInteger;
...
procedure TFrmConsultarPedData.FormShow(Sender: TObject);
begin
  DTPickerIni.Date:=Now -5;
  DTPickerFin.Date:=Now;
end;

procedure TFrmConsultarPedData.DBGrid1DrawColumnCell(Sender: TObject;
...
procedure TFrmConsultarPedData.PNGButton8Click(Sender: TObject);
begin
  if  IBQPedidos.Active then
    IBQPedidos.First;
end;

procedure TFrmConsultarPedData.PNGButton7Click(Sender: TObject);
begin
  if  IBQPedidos.Active then
    IBQPedidos.last;
end;
