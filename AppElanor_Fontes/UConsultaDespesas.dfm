object FrmConsultarDespesas: TFrmConsultarDespesas
  Left = 343
  Top = 154
  AutoScroll = False
  BorderIcons = []
  ClientHeight = 531
  ClientWidth = 920
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 920
    Height = 97
    Align = alTop
    TabOrder = 0
    object PNGButton6: TPNGButton
      Left = 784
      Top = 20
      Width = 57
      Height = 65
      ButtonLayout = pbsImageAbove
      Caption = 'Imprimir'
      ImageNormal.Data = {
        
        180000000049454E44AE426082}
      ButtonStyle = pbsFlat
      OnClick = PNGButton6Click
    end
    object PNGButton2: TPNGButton
      Left = 848
      Top = 20
      Width = 57
      Height = 65
      ButtonLayout = pbsImageAbove
      Caption = 'Voltar'
      ImageNormal.Data = {
        
        426082}
      ButtonStyle = pbsFlat
      OnClick = PNGButton2Click
    end
    object PNGButton1: TPNGButton
      Left = 720
      Top = 20
      Width = 57
      Height = 65
      ButtonLayout = pbsImageAbove
      Caption = 'Localizar'
      ImageNormal.Data = {
        
        952968310000000049454E44AE426082}
      ButtonStyle = pbsFlat
      OnClick = PNGButton1Click
    end
    object Label2: TLabel
      Left = 472
      Top = 32
      Width = 46
      Height = 13
      Caption = 'Per'#237'odo'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label3: TLabel
      Left = 64
      Top = 32
      Width = 30
      Height = 13
      Caption = 'Total'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object DTPickerIni: TDateTimePicker
      Left = 472
      Top = 48
      Width = 97
      Height = 21
      Date = 43836.525099490740000000
      Time = 43836.525099490740000000
      TabOrder = 0
    end
    object DTPickerFin: TDateTimePicker
      Left = 600
      Top = 48
      Width = 97
      Height = 21
      Date = 43836.525188726850000000
      Time = 43836.525188726850000000
      TabOrder = 1
    end
    object EdtTotal: TEdit
      Left = 64
      Top = 48
      Width = 345
      Height = 19
      Ctl3D = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentCtl3D = False
      ParentFont = False
      ReadOnly = True
      TabOrder = 2
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 120
    Width = 920
    Height = 217
    TabOrder = 1
    object DBGrid1: TDBGrid
      Left = 1
      Top = 1
      Width = 918
      Height = 192
      Hint = 
        'Clique duas vezes no pedido desejado para exibir os produtos aba' +
        'ixo'
      Ctl3D = False
      DataSource = DDsBoletos
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentCtl3D = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = [fsBold]
      Columns = <
        item
          Expanded = False
          FieldName = 'TBCONT_DATAPAG'
          Title.Caption = 'Data'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'TBFOR_NOME'
          Title.Caption = 'Fornecedor'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'TBCONT_VALOR'
          Title.Caption = 'Valor'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'TBCONT_NUMNOTA'
          Title.Caption = 'Nota fiscal'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'TBCONT_VENCIMENTO'
          Title.Caption = 'Vencimento'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'TBCONT_EMISSAO'
          Title.Caption = 'Emiss'#227'o'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'TBCONT_DESCRICAO'
          Title.Caption = 'Descri'#231#227'o'
          Visible = True
        end>
    end
    object Panel3: TPanel
      Left = 1
      Top = 193
      Width = 918
      Height = 23
      Align = alBottom
      TabOrder = 1
      object Label4: TLabel
        Left = 404
        Top = 6
        Width = 56
        Height = 13
        Caption = 'Despesas'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        Layout = tlCenter
      end
    end
  end
  object PnlItens: TPanel
    Left = 0
    Top = 336
    Width = 920
    Height = 195
    Align = alBottom
    TabOrder = 2
    object DBGrid2: TDBGrid
      Left = 1
      Top = 1
      Width = 918
      Height = 193
      Align = alClient
      Ctl3D = False
      DataSource = DSDespesas
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = [fsBold]
      Columns = <
        item
          Expanded = False
          FieldName = 'TBG_DATA'
          Title.Caption = 'Data'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'TBFOR_NOME'
          Title.Caption = 'Fornecedor'
          Width = 250
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'TBG_VALOR'
          Title.Caption = 'Valor'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'TBG_DESCRICAO'
          Title.Caption = 'Descri'#231#227'o'
          Width = 411
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'TBG_EMISSAO'
          Title.Caption = 'Emiss'#227'o'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'TBG_NUMNOTA'
          Title.Caption = 'Nota fiscal'
          Visible = True
        end>
    end
  end
  object Panel4: TPanel
    Left = 0
    Top = 97
    Width = 920
    Height = 25
    Align = alTop
    TabOrder = 3
    object Label1: TLabel
      Left = 404
      Top = 6
      Width = 43
      Height = 13
      Caption = 'Boletos'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      Layout = tlCenter
    end
  end
  object IBQDespesas: TIBQuery
    Database = FrmPrincipal.IBDMain
    Transaction = FrmPrincipal.IBTMain
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT         TBG_VALOR,'
      '               TBG_DATA,'
      '               TB_GASTOS.ID_FORNECEDOR,'
      '               TB_FORNECEDORES.ID_FORNECEDOR,'
      '               TBG_DESCRICAO,'
      '               TBFOR_NOME,'
      '               TBG_EMISSAO,'
      '               TBG_NUMNOTA'
      'FROM TB_GASTOS'
      'INNER JOIN TB_FORNECEDORES'
      'ON TB_FORNECEDORES.ID_FORNECEDOR=TB_GASTOS.ID_FORNECEDOR'
      'WHERE TBG_DATA BETWEEN :pDataIni AND :pDataFin'
      'ORDER BY TBG_DATA')
    Left = 72
    Top = 369
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'pDataIni'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'pDataFin'
        ParamType = ptUnknown
      end>
    object IBQDespesasTBG_VALOR: TIBBCDField
      FieldName = 'TBG_VALOR'
      Origin = 'TB_GASTOS.TBG_VALOR'
      currency = True
      Precision = 18
      Size = 3
    end
    object IBQDespesasTBG_DATA: TDateField
      FieldName = 'TBG_DATA'
      Origin = 'TB_GASTOS.TBG_DATA'
    end
    object IBQDespesasID_FORNECEDOR: TIntegerField
      FieldName = 'ID_FORNECEDOR'
      Origin = 'TB_GASTOS.ID_FORNECEDOR'
    end
    object IBQDespesasTBG_DESCRICAO: TIBStringField
      FieldName = 'TBG_DESCRICAO'
      Origin = 'TB_GASTOS.TBG_DESCRICAO'
      Size = 80
    end
    object IBQDespesasTBFOR_NOME: TIBStringField
      FieldName = 'TBFOR_NOME'
      Origin = 'TB_FORNECEDORES.TBFOR_NOME'
      Size = 60
    end
    object IBQDespesasTBG_EMISSAO: TDateField
      FieldName = 'TBG_EMISSAO'
      Origin = 'TB_GASTOS.TBG_EMISSAO'
    end
    object IBQDespesasTBG_NUMNOTA: TIBStringField
      FieldName = 'TBG_NUMNOTA'
      Origin = 'TB_GASTOS.TBG_NUMNOTA'
      Size = 12
    end
  end
  object DDsBoletos: TDataSource
    DataSet = IBQBoletos
    Left = 320
    Top = 121
  end
  object IBQUtil: TIBQuery
    Database = FrmPrincipal.IBDMain
    Transaction = FrmPrincipal.IBTMain
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT    TB_PEDIDOS.ID_PEDIDO,'
      '          TBPED_DATA,'
      '          TB_PEDIDOS.id_cliente,'
      '          TB_PEDIDOS.TBPED_NOME,'
      '          TBPED_ENDERECO,'
      '          TBPED_CIDADE,'
      '          TBPED_ESTADO,'
      '          TBPED_TELEFONE,'
      '          TB_PEDIDOS.ID_PRAZO,'
      '          TBPED_VALTOTAL,'
      '          TBPED_VENC01,'
      '          TBPED_VENC02,'
      '          TBPED_VENC03,'
      '          TB_PEDIDOS.ID_USUARIO,'
      '          TBPED_BAIRRO,'
      '          TBPED_CNPJ,'
      '           TB_PRAZOS.TBPRZ_NOME,'
      '           TBPED_NUMPED,'
      '           TB_USUARIO.TBUSR_NOME,'
      '           TBPED_CANCELADO,'
      '           TBPED_MOTIVOCANCEL,'
      '            OBS'
      'FROM TB_PEDIDOS'
      'INNER JOIN TB_PRAZOS'
      'ON (TB_PRAZOS.ID_PRAZO=TB_PEDIDOS.ID_PRAZO)'
      'INNER JOIN TB_USUARIO'
      'ON TB_USUARIO.ID_USUARIO=TB_PEDIDOS.ID_USUARIO'
      'WHERE TB_PEDIDOS.TBPED_NOME=:pNomeCli'
      'AND TBPED_DATA BETWEEN :pDataIni AND :pDataFin'
      'ORDER BY TBPED_NUMPED'
      '')
    Left = 224
    Top = 105
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'pNomeCli'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'pDataIni'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'pDataFin'
        ParamType = ptUnknown
      end>
  end
  object IBQBoletos: TIBQuery
    Database = FrmPrincipal.IBDMain
    Transaction = FrmPrincipal.IBTMain
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT  TBCONT_NUMNOTA,'
      '        TBCONT_EMISSAO,'
      '        TBCONT_VALORNOTA,'
      '        TBCONT_VENCIMENTO,'
      '        TBCONT_VALOR,'
      '        TBCONT_STATUS,'
      '        TBCONT_DATAPAG, '
      '        TBCONT_DESCRICAO,'
      '        TBFOR_NOME'
      'FROM TB_CONTASPAGAR        '
      'INNER JOIN TB_FORNECEDORES'
      'ON TB_FORNECEDORES.ID_FORNECEDOR= TB_CONTASPAGAR.ID_FORNECEDOR'
      'WHERE TBCONT_DATAPAG BETWEEN :pDataIni AND :pDataFin         '
      'ORDER BY   TBCONT_DATAPAG')
    Left = 320
    Top = 89
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'pDataIni'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'pDataFin'
        ParamType = ptUnknown
      end>
    object IBQBoletosTBCONT_NUMNOTA: TIBStringField
      FieldName = 'TBCONT_NUMNOTA'
      Origin = 'TB_CONTASPAGAR.TBCONT_NUMNOTA'
      Size = 12
    end
    object IBQBoletosTBCONT_EMISSAO: TDateField
      FieldName = 'TBCONT_EMISSAO'
      Origin = 'TB_CONTASPAGAR.TBCONT_EMISSAO'
    end
    object IBQBoletosTBCONT_VALORNOTA: TIBBCDField
      FieldName = 'TBCONT_VALORNOTA'
      Origin = 'TB_CONTASPAGAR.TBCONT_VALORNOTA'
      Precision = 18
      Size = 2
    end
    object IBQBoletosTBCONT_VENCIMENTO: TDateField
      FieldName = 'TBCONT_VENCIMENTO'
      Origin = 'TB_CONTASPAGAR.TBCONT_VENCIMENTO'
    end
    object IBQBoletosTBCONT_VALOR: TIBBCDField
      FieldName = 'TBCONT_VALOR'
      Origin = 'TB_CONTASPAGAR.TBCONT_VALOR'
      currency = True
      Precision = 18
      Size = 2
    end
    object IBQBoletosTBCONT_STATUS: TIBStringField
      FieldName = 'TBCONT_STATUS'
      Origin = 'TB_CONTASPAGAR.TBCONT_STATUS'
      FixedChar = True
      Size = 2
    end
    object IBQBoletosTBCONT_DATAPAG: TDateField
      FieldName = 'TBCONT_DATAPAG'
      Origin = 'TB_CONTASPAGAR.TBCONT_DATAPAG'
    end
    object IBQBoletosTBCONT_DESCRICAO: TIBStringField
      FieldName = 'TBCONT_DESCRICAO'
      Origin = 'TB_CONTASPAGAR.TBCONT_DESCRICAO'
      Size = 80
    end
    object IBQBoletosTBFOR_NOME: TIBStringField
      FieldName = 'TBFOR_NOME'
      Origin = 'TB_FORNECEDORES.TBFOR_NOME'
      Size = 60
    end
  end
  object DSDespesas: TDataSource
    DataSet = IBQDespesas
    Left = 72
    Top = 401
  end
  object CDSRelDespesas: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 384
    Top = 168
    object CDSRelDespesasData: TDateField
      FieldName = 'Data'
    end
    object CDSRelDespesasFornecedor: TStringField
      FieldName = 'Fornecedor'
      Size = 80
    end
    object CDSRelDespesasValor: TCurrencyField
      FieldName = 'Valor'
    end
    object CDSRelDespesasNota: TStringField
      FieldName = 'Nota'
      Size = 12
    end
    object CDSRelDespesasVencimento: TDateField
      FieldName = 'Vencimento'
    end
    object CDSRelDespesasBoleto: TStringField
      FieldName = 'Boleto'
      Size = 2
    end
    object CDSRelDespesasDescricao: TStringField
      FieldName = 'Descricao'
      Size = 80
    end
  end
  object DSRelDespesas: TDataSource
    DataSet = CDSRelDespesas
    Left = 416
    Top = 168
  end
end
